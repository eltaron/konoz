...
@push('styles')
    ...
    <style>
        ...
        .juz-card:hover {
            border-color: rgba(15, 109, 128, 0.15);
            box-shadow: 0 2px 12px rgba(15, 109, 128, 0.06);
        ...
        .juz-reviewing {
            border-color: rgba(245, 189, 88, 0.2);
            background: rgba(245, 189, 88, 0.03);
        }

        .juz-reviewing::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
...
@section('content')
    <h1 class="fw-bold mb-1" style="color: #0F6D80; font-size: 1.6rem;" data-aos="fade-up">
        {{ __('messages.student_hifdh_title') }}</h1>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="dash-card" data-aos="fade-up">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    ...
                    <div class="d-flex align-items-center gap-2 small">
                        <span class="d-flex align-items-center gap-1"><span class="rounded-1 d-inline-block"
                                style="width:10px;height:10px;background:#27ae60;"></span>
                            {{ __('messages.student_hifdh_completed') }}</span>
                        <span class="d-flex align-items-center gap-1"><span class="rounded-1 d-inline-block"
                                style="width:10px;height:10px;background:var(--pumpkin);"></span>
                            {{ __('messages.student_hifdh_reviewing') }}</span>
                        <span class="d-flex align-items-center gap-1"><span class="rounded-1 d-inline-block"
                                style="width:10px;height:10px;background:#0F6D80;"></span>
                            {{ __('messages.student_hifdh_in_progress') }}</span>
                        <span class="d-flex align-items-center gap-1"><span class="rounded-1 d-inline-block"
                                style="width:10px;height:10px;background:#d0d9db;"></span>
                            {{ __('messages.student_hifdh_not_started') }}</span>
                    </div>
                </div>
                <div class="row g-2">
                    @foreach ($juzProgress as $jp)
                        @php
                            $daysUntilReview = $jp->target_review_at
                                ? now()->startOfDay()->diffInDays($jp->target_review_at, false)
                                : null;
                            $reviewClass =
                        ...
                        <div class="col-4 col-md-2">
                            <div class="juz-card juz-{{ $jp->status }} {{ $reviewClass }}"
                                onclick="openJuzModal({{ $jp->id }}, {{ $jp->juz_number }}, '{{ $jp->status }}', '{{ $jp->started_at?->format('Y-m-d') }}', '{{ $jp->target_review_at?->format('Y-m-d') }}', '{{ addslashes($jp->notes ?? '') }}')"
                                data-bs-toggle="tooltip" title="{{ __('messages.student_click_adjust') }}">
                                <div class="juz-card-header">
                                    ...
                                    <span class="juz-card-icon">
                                        ...
                                        @elseif($jp->status === 'reviewing')
                                            <i class="fa-solid fa-rotate" style="color:var(--pumpkin);"></i>
                                        @elseif($jp->status === 'in_progress')
                                            <i class="fa-solid fa-book-open" style="color:#0F6D80;"></i>
                                        @else
                                            <i class="fa-solid fa-lock" style="color:#bec8cc;"></i>
                                        @endif
                                    </span>
                                </div>
                                @if ($jp->started_at)
                                ...
                                @if ($jp->target_review_at)
                                    <span
                                        class="juz-card-date {{ $daysUntilReview < 0 ? 'text-danger' : ($daysUntilReview <= 3 ? 'text-warning' : '') }}"
                                        style="font-size:0.58rem;">
                                        <i class="fa-regular fa-clock"></i>
                                        @if ($daysUntilReview < 0)
                                            {{ __('messages.student_overdue') }}
                                            {{ abs($daysUntilReview) }}{{ __('messages.student_days') }}
                                        @else
                                            {{ __('messages.student_after') }}
                                            {{ $daysUntilReview }}{{ __('messages.student_days') }}
                                        @endif
                                    </span>
                                @endif
                            </div>
            ...
            <div class="dash-card mt-4" data-aos="fade-up">
                <h5 class="fw-bold mb-3" style="color: #0F6D80;">
                    ...
                    <small class="text-secondary opacity-50">({{ $todayActivities->count() }}
                        {{ __('messages.student_hifdh_activities_today') }})</small>
                </h5>
                    ...
                    <table class="table align-middle mb-0" style="font-size:0.85rem;">
                        <thead>
                            <tr style="color:#0F6D80;border-bottom:2px solid rgba(15,109,128,0.1);">
                                ...
                                <th class="fw-bold text-center" style="width:70px;">
                                    {{ __('messages.student_hifdh_parts_col') }}</th>
                                <th class="fw-bold text-center" style="width:60px;">
                                    {{ __('messages.student_hifdh_duration_col') }}</th>
                                <th class="fw-bold text-center" style="width:40px;">
                                    {{ __('messages.student_hifdh_status_col') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                            ...
                            @foreach ($weekDays as $i => $day)
                                ...
                                <tr class="{{ $isToday ? 'table-active' : '' }}"
                                    style="{{ $isToday ? 'background:rgba(15,109,128,0.04);' : '' }}{{ $isFuture ? 'opacity:0.35;' : '' }}">
                                    <td class="ps-0">
                                        <span class="fw-bold small">{{ $day }}</span>
                                        <br><small class="text-secondary opacity-50"
                                            style="font-size:0.6rem;">{{ \Carbon\Carbon::parse($date)->format('d/m') }}</small>
                                    </td>
                                    <td>
                                        @if ($dayActs->isEmpty() && !$isFuture)
                                            <span class="small text-secondary opacity-50"
                                                onclick="logActivityForDay('{{ $date }}','{{ $day }}')"
                                                style="cursor:pointer;">—
                                                {{ __('messages.student_hifdh_click_log') }}</span>
                                        @elseif($isFuture)
                                            <span
                                                class="small text-secondary opacity-25">{{ __('messages.student_hifdh_not_yet') }}</span>
                                        @else
                                            <div class="d-flex flex-wrap gap-1">
                                                @foreach ($dayActs as $act)
                                                    <span
                                                        class="d-inline-flex align-items-center gap-1 small px-2 py-1 rounded-2"
                                                        style="background:rgba(15,109,128,0.06);color:#0F6D80;font-size:0.7rem;">
                                                        <i
                                                        ...
                                                        {{ $act->activity_type }}
                                                        @if ($act->juz_number)
                                                            <span style="font-size:0.6rem;">{{ $act->juz_number }}</span>
                                                        @endif
                                                    </span>
                                                @endforeach
                                            </div>
                                    ...
                                    <td class="text-center">
                                        @if ($juzList->isNotEmpty())
                                            <span class="small fw-medium"
                                                style="color:#0F6D80;">{{ $juzList->implode('، ') }}</span>
                                        @else
        ...
        <div class="col-lg-4">
            <div class="dash-card mb-4" data-aos="fade-up">
                <h5 class="fw-bold mb-3" style="color: #0F6D80;">{{ __('messages.student_hifdh_streak_title') }} <small
                        class="text-secondary opacity-50">({{ $streakDays }}
                        {{ __('messages.student_hifdh_day') }})</small></h5>
            ...
            <div class="dash-card mb-4" data-aos="fade-up">
                ...
                <div id="weekStats">
                    @php
                        $typeLabels = [
                            __('messages.activity_memorize') => '#0F6D80',
                            __('messages.activity_review') => '#f39c12',
                            __('messages.activity_recite') => '#27ae60',
                            __('messages.activity_exam') => '#e74c3c',
                            __('messages.activity_listen') => '#8e44ad',
                        ];
                        $total = $activityCounts->sum();
                    @endphp
                    @if ($total === 0)
                        <p class="small text-secondary opacity-75 text-center py-3">
                            {{ __('messages.student_hifdh_no_achievements') }}</p>
                    @else
                        @foreach ($typeLabels as $type => $color)
                            @if (($activityCounts[$type] ?? 0) > 0)
                                <div class="d-flex align-items-center gap-2 mb-2 small">
                                    <i class="fa-solid {{ $type === __('messages.activity_memorize') ? 'fa-book-quran' : ($type === __('messages.activity_review') ? 'fa-rotate' : ($type === __('messages.activity_recite') ? 'fa-microphone' : ($type === __('messages.activity_exam') ? 'fa-file-pen' : ($type === __('messages.activity_listen') ? 'fa-headphones' : 'fa-circle')))) }}"
                                        style="color:{{ $color }};width:16px;"></i>
                                    <span class="flex-grow-1">{{ $type }}</span>
                                    <span class="fw-bold"
                                        style="color:{{ $color }};">{{ $activityCounts[$type] }}</span>
                                </div>
                            @endif
                        @endforeach
                        @if ($weeklyMinutes ?? false)
                            ...
                            <div class="d-flex justify-content-between small">
                                <span>{{ __('messages.student_hifdh_total_minutes') }}</span><span class="fw-bold"
                                    style="color:#0F6D80;">{{ $weeklyMinutes }} د</span></div>
                        @endif
                    @endif
                </div>
            </div>
                ...
                <div class="d-flex flex-column gap-3">
                    <div>
                        <div class="d-flex justify-content-between small mb-1">
                            <span>{{ __('messages.student_hifdh_new_memorization') }}</span><span>{{ $hifdhCount }}/5</span>
                        </div>
                        <div class="progress" style="height: 6px; background: #e0e3e4;">
                            <div class="progress-bar rounded-3"
                                style="width: {{ min(($hifdhCount / 5) * 100, 100) }}%; background: #0F6D80;"></div>
                        </div>
                    </div>
                    <div>
                        <div class="d-flex justify-content-between small mb-1">
                            <span>{{ __('messages.student_hifdh_review') }}</span><span>{{ $reviewCount }}/3</span></div>
                        <div class="progress" style="height: 6px; background: #e0e3e4;">
                            <div class="progress-bar rounded-3"
                                style="width: {{ min(($reviewCount / 3) * 100, 100) }}%; background: var(--pumpkin);"></div>
                        </div>
                    </div>
                    <div>
                        <div class="d-flex justify-content-between small mb-1">
                            <span>{{ __('messages.student_hifdh_test') }}</span><span>{{ $testCount }}/1</span></div>
                        <div class="progress" style="height: 6px; background: #e0e3e4;">
                            <div class="progress-bar rounded-3"
                                style="width: {{ $testCount * 100 }}%; background: #0F6D80;"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="dash-card" data-aos="fade-up">
                ...
                <div class="d-flex flex-column gap-2">
                    <button class="btn w-100 rounded-3 py-2" style="background:#0F6D80;color:#fff;"
                        onclick="logQuickActivity('{{ __('messages.activity_memorize') }}', 30)"><i
                            class="fa-solid fa-book-quran ml-1"></i>{{ __('messages.student_hifdh_memorize_30') }}</button>
                    <button class="btn w-100 rounded-3 py-2" style="background:var(--pumpkin);color:#5f4100;"
                        onclick="logQuickActivity('{{ __('messages.activity_review') }}', 20)"><i
                            class="fa-solid fa-rotate ml-1"></i>{{ __('messages.student_hifdh_review_20') }}</button>
                    <button class="btn w-100 rounded-3 py-2"
                        style="background:rgba(39,174,96,0.1);color:#27ae60;border:1px solid rgba(39,174,96,0.2);"
                        onclick="logQuickActivity('{{ __('messages.activity_recite') }}', 15)"><i
                            class="fa-solid fa-microphone ml-1"></i>{{ __('messages.student_hifdh_recite_15') }}</button>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content border-0" style="border-radius:16px;">
                ...
                <div class="modal-body px-4 pb-3">
                    <form id="juzForm">
                        ...
                        <div class="mb-3">
                            <label
                                class="small fw-bold mb-1 text-secondary">{{ __('messages.student_juz_modal_status') }}</label>
                        ...
                        <div class="row g-2 mb-3">
                            <div class="col-6">
                                <label
                                    class="small fw-bold mb-1 text-secondary">{{ __('messages.student_juz_start_date') }}</label>
                                <input type="date" id="juzStart" class="form-control form-control-sm"
                                    style="border-radius:10px;">
                            </div>
                            <div class="col-6">
                                <label
                                    class="small fw-bold mb-1 text-secondary">{{ __('messages.student_juz_review_date') }}</label>
                                <input type="date" id="juzReview" class="form-control form-control-sm"
                                    style="border-radius:10px;">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label
                                class="small fw-bold mb-1 text-secondary">{{ __('messages.student_juz_notes') }}</label>
                            <textarea id="juzNotes" class="form-control form-control-sm" rows="2" style="border-radius:10px;"
                                placeholder="{{ __('messages.student_juz_notes_placeholder') }}"></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button class="btn px-4 py-2 fw-bold" style="background:#0F6D80;color:#fff;border-radius:10px;"
                        onclick="saveJuzDates()">{{ __('messages.student_juz_save') }}</button>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        const weekDays = ['{{ __('messages.saturday') }}', '{{ __('messages.sunday') }}', '{{ __('messages.monday') }}',
            '{{ __('messages.tuesday') }}', '{{ __('messages.wednesday') }}', '{{ __('messages.thursday') }}',
            '{{ __('messages.friday') }}'
        ];
        const dayShort = ['{{ __('messages.sat_short') }}', '{{ __('messages.sun_short') }}',
            '{{ __('messages.mon_short') }}', '{{ __('messages.tue_short') }}', '{{ __('messages.wed_short') }}',
            '{{ __('messages.thu_short') }}', '{{ __('messages.fri_short') }}'
        ];
        ...
        function logActivityForDay(dateStr, dayName) {
            ...
            Swal.fire({
                ...
                preConfirm: function(v) {
                    if (locale === 'ar') {
                        var optMap = {
                            memorize: actTypes.memorize,
                            review: actTypes.review,
                            recite: actTypes.recite,
                            exam: actTypes.exam,
                            listen: actTypes.listen
                        };
                        var found = Object.values(optMap).find(function(o) {
                            return o === v;
                        });
                        if (!found) {
                            Swal.showValidationMessage('{{ __('messages.hifdh_validation_select') }}');
                            return false;
                        }
                        return v;
                    } else {
                ...
                didOpen: function() {
                    ...
                    if (sel) {
                        sel.innerHTML = '<option value="">' + '{{ __('messages.hifdh_select_prompt') }}' +
                            '</option>' +
                            Object.keys(selectOptions).map(function(k) {
                                return '<option value="' + k + '">' + selectOptions[k] + '</option>';
                            }).join('');
                        sel.addEventListener('change', function() {
                            document.getElementById('juzPickerWrap').style.display = (this.value ===
                                    actTypes.memorize || this.value === actTypes.review) ? 'block' :
                                'none';
                        });
                    }
                }
                ...
                if (r.isConfirmed) {
                    var duration = document.getElementById('swalDuration') ? document.getElementById('swalDuration')
                        .value : '';
        ...
        function logQuickActivity(type, minutes) {
            ...
            if (type === actTypes.memorize || type === actTypes.review) {
                ...
                Swal.fire({
                    ...
                    html: '<select id="swalJuzQuick" class="form-select"><option value="">{{ __('messages.hifdh_select_juz_short') }}</option>' +
                        juzOptions + '</select>',
                    showCancelButton: true,
                    confirmButtonColor: '#0F6D80',
                    confirmButtonText: '{{ __('messages.hifdh_save_success') }}',
                    ...
                    preConfirm: function() {
                        ...
                        if (!juz) {
                            Swal.showValidationMessage('{{ __('messages.hifdh_validation_select_juz') }}');
                            return false;
                        }
                        return juz;
                    }
        ...
        function saveActivity(dateStr, type, minutes, juzNumber) {
            ...
            fetch('{{ route('student.activity.log') }}', {
                ...
                Swal.fire({
                    icon: 'error',
                    title: '{{ __('messages.hifdh_save_error_title') }}',
                    text: '{{ __('messages.hifdh_save_error') }}'
                });
            });
        }
